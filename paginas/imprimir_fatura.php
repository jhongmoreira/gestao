...
<div class="container-fluid">
        ...
        <tbody>
          <tr>
            ...
            ?>
                ...
                <td><?php echo 'R$ ' . number_format($dados['valor_uni'] * $dados['qnt'], 2, ',', '.'); ?></td>
          </tr>
      <?php
              }
            } else {
  ...
  <div class="row float-right">
    <div class="col-md-12">
      ...
      <?php
      $banco->query("SELECT SUM(valor_uni*qnt) as total_soma FROM vendas, servicos, clientes WHERE vendas.cliente = clientes.id AND vendas.pago = 0 AND vendas.servico = servicos.id_servico AND clientes.id = $idCli AND data_venda BETWEEN '$dataInicial' AND '$dataFim'");
      foreach ($banco->result() as $dados) {
      }
      ...
      <div>
        ...
        $total = $banco->linhas();
        ?>
        <h5 class="mt-5 mb-2 text-center"></h5>
        <?php
        if ($total != 0) {
          foreach ($banco->result() as $dados) {
            $img_upload = $dados['img'];
            echo "<img src='upload/$img_upload' width='20%'>";
        ?>
        <?php
          }
        } else {
